...
# remove locations that are outside of england
DELETE FROM family_location
WHERE location_code >9;
...
# removing data where location data is missing
DELETE FROM health_condition
WHERE MCS_ID NOT IN (SELECT f.MCS_ID
FROM family_location f);
...
DELETE FROM reported_health_factors
WHERE MCS_ID NOT IN (SELECT f.MCS_ID
FROM family_location f);
